...
#include <semaphore.h> //Adicionado

//Definições adicionadas
#define NUM_ELEVATORS 2
#define BUILDING_FLOORS 7 //Andares 0 a 6

sem_t limite_passageiros; //Semáforo
//COMUNICAÇÃO ENTRE THREADS
pthread_cond_t cond = PTHREAD_COND_INITIALIZER; //Adicionado
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER; //Adicionado
...
typedef struct {
  ...
  short elevator_id;//Adicionado
  
  pthread_mutex_t lock; //Adicionado
  pthread_cond_t cond; //Adicionado

} Elevator;

//Adicionado, nova estrutura, para o 'predio'
typedef struct {
...
//Adicionado: sistema de semáforo
...
//Thread para um elevador genérico, SUBSTITUI 'move_elevator'
void *elevator_thread(void *args) {
    ...
    // Movimentação do elevador até o andar requisitado
...
// aleatoriamente. Nesse caso, a estrutura de entrada será Building
void *request(void *args) {
  ...
  sleep(rand() % 30 + 1);  // Simula um tempo aleatório entre requisições
...
int select_elevator(Building *building, short requested_floor) {
    ...
    //Critério de prioridade: elevador mais próximo que não está se movendo
    for (int i = 0; i < NUM_ELEVATORS; i++) {
...
//Função para semaforo
void *enter_elevator(void *args) {
    sem_wait(&limite_passageiros); //Espera enquanto elevador está lotado
    
    printf("Passageiro entrou no elevador.\n");
    sem_post(&limite_passageiros);  //Libera o espaço após o passageiro sair
    return NULL;
}
